...
def get_save_dir(m):
    """尝试拿到本次 val 输出目录（runs/detect/val*） 不同版本字段名可能不同，做容错。.
    """
...
def main():
    ...
    for name, yaml_path in DATASETS.items():
        ...
        rows.append({"dataset": name, "P": P, "R": R, "mAP50": mAP50, "mAP50-95": mAP5095, "val_output_dir": save_dir})

    with open(summary_path, "w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(f, fieldnames=["dataset", "P", "R", "mAP50", "mAP50-95", "val_output_dir"])
        writer.writeheader()
